...
Build a modular Python pipeline for F1 aerodynamics analysis that connects first-principles analytical models to real FastF1 telemetry data. Output a standalone Mercedes-themed HTML portfolio site on GitHub Pages, with the same delivery style as the Soccer CFD and Airfoil CFD projects. SU2 CFD validation is included for the floor venturi tunnel ground effect simulation.
...
```bash
uv run python -m runners.all                  # run all telemetry/analytical analyses
...
<｜｜DSML｜｜parameter name="description" string="true">How to run the project, SU2 config quirks, FastF1 telemetry