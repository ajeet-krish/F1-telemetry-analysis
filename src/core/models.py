...
class FrontWingConfig:
    ...
    # Multi-element slot effect raises effective cl_alpha well beyond 2π
    cl_alpha: float = 8.0  # lift curve slope (per radian, includes multi-element slot effect)
...
class RearWingConfig:
    ...
    span: float = 0.95  # m (between rear wheel centreline)
    chord: float = 0.35  # m (mean aerodynamic chord)
    area: float = 0.35  # m² (2022+ main plane + flap)
    max_angle: float = 20.0  # degrees (including DRS actuation range)

    # Coefficients (calibrated against CFD)
    cl_alpha: float = 8.5  # per radian (multi-element, operates in floor downwash)
...
class FloorConfig:
    ...
    cl_base: float = -0.5  # base lift coefficient at infinite ride height
...
class Floor:
    ...
    def cl(self, ride_height: float, velocity_ms: float = 80.0) -> float:
        ...
        # Yields C_L ~ -1.6 at 100mm, ~ -2.5 at 50mm, ~ -4.6 at 25mm
        A = 0.05  # linear ground effect term
        B = 0.001  # quadratic term (stronger at very low h)

        ground_multiplier = 1.0 + A / h + B / (h**2)

        # Diffuser contribution: small pressure recovery effect
        diffuser_factor = 1.0
...
class F1Car:
    ...
    def total_downforce(
        self,
        velocity_ms: float,
        front_alpha: float = -6.0,
        rear_alpha: float = -15.0,
        ...
        q = dynamic_pressure(velocity_ms)
        df_body = 0.12 * q * self.cfg.frontal_area  # body/nose/floor interaction
    ...
    def component_breakdown(self, velocity_ms: float, **kwargs) -> dict:
        ...
        df_fw = self.front_wing.downforce(
            velocity_ms,
            kwargs.get("front_alpha", -6.0),
            kwargs.get("ride_height", 0.05),
        )
        df_rw = self.rear_wing.downforce(
            velocity_ms, kwargs.get("rear_alpha", -15.0), kwargs.get("drs_open", False)
        )
        df_floor = self.floor.downforce(velocity_ms, kwargs.get("ride_height", 0.05))
        q = dynamic_pressure(velocity_ms)
        df_body = 0.12 * q * self.cfg.frontal_area
